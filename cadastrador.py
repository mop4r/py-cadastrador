...
class UserSearchDialog(QDialog):
    ...
    def copy_menus_for_user(self):
        ...
        if not self.has_custom_menus(selected_user_id):
            QMessageBox.warning(self, 'Aviso', 'Usuário selecionado não possui menus personalizados cadastrados, selecione outro.')
            return
...
class MainWindow(QMainWindow):
    ...
    def insert_new_user(self, username, use_existing_password, company_code):
        ...
        try:
            ...
            insert_query = f"INSERT INTO \"DBA\".\"USUARIOS\" (\"CODIGOEMPRESA\",\"USUARIO\",\"SENHA\",\"NIVEL\",\"SEQUENCIA_USU\",\"CODIGOUNICO\",\"STATUS\",\"SENHAWEB\",\"DATACAD\",\"Pw_Adic\") VALUES({company_code}, '{username}', '{password_hash}', {user_level}, {new_sequence}, 0, 'ATIVO', NULL, (select getdatanumerohoje()), 'NNNNNNNNNN')"
            cursor.execute(insert_query)
            self.conn.commit()
    ...
    def insert_user_in_specific_locations(self, selected_locations):
        ...
        try:
            ...
            for location in selected_locations:
                insert_query = f"INSERT INTO \"DBA\".\"GES_094\" (\"EMPRESA\",\"ULV001\",\"ULV002\",\"ULV003\",\"SEQUENCIA_USU\",\"ULV004\") VALUES({company_code},'{username}',{location['cod_local']},0,(SELECT MAX(SEQUENCIA_USU) FROM \"DBA\".\"USUARIOS\"),NULL)"
                cursor.execute(insert_query)

            # Get the user ID of the newly inserted user
            user_id_query = f"SELECT SEQUENCIA_USU FROM \"DBA\".\"USUARIOS\" WHERE \"USUARIO\" = '{username}' AND \"CODIGOEMPRESA\" = {company_code}"
            cursor.execute(user_id_query)
            self.current_user_id = cursor.fetchone()[0]  # Save the user ID for later use
            ...
            if copy_menus_reply == 0:  # Se o botão "Copiar menus" for pressionado
                self.user_search_dialog = UserSearchDialog(self.conn, self.current_user_id)
            ...
            if gestor_os_reply == 0:  # Se o botão "Sim" for pressionado
                if OSProfileManager.has_registered_profiles_for_company(self.conn, company_code):
                    self.os_profile_manager = OSProfileManager(self.conn, self.current_user_id, company_code,
                                                               selected_locations)
